...
pub fn handler(
    ...
    a_to_b: bool,  // false for USDC→SOL (B to A)
) -> Result<()> {
    let vault_usdc_amount = ctx.accounts.vault_usdc_ata.amount;
    ...
    require!(
        vault_usdc_amount >= amount_in,
        ErrorCode::InsufficientUserBalance
    );
    
    require!(
        token_vault_total_usdc >= amount_in,
        ErrorCode::InsufficientUserBalance
    );
    ...
    msg!("Preparing to swap {} USDC for SOL", amount_in);
    msg!("Minimum SOL out: {}", minimum_amount_out);
    
    let vault_pda_key = ctx.accounts.vault_pda.key();
    
    // Create swap instruction for Orca
    // Note: For USDC→SOL, a_to_b should be false since USDC is token B
    let ix = anchor_lang::solana_program::instruction::Instruction {
        ...
        data: {
            ...
            data.push(0xf8); // Swap discriminator
    ...
    let wsol_received = ctx.accounts.vault_wsol_ata.amount.saturating_sub(token_vault_total_wsol);
    
    require!(
        wsol_received >= minimum_amount_out,
    ...
    token_vault.total_usdc = token_vault_total_usdc.checked_sub(amount_in)
        .ok_or(ErrorCode::MathOverflow)?;
    token_vault.total_wsol = ctx.accounts.vault_wsol_ata.amount;
    
    // Update swap state
    swap_state.total_usdc_received = swap_state.total_usdc_received.checked_sub(amount_in)
        .ok_or(ErrorCode::MathOverflow)?;
    swap_state.total_sol_swapped = swap_state.total_sol_swapped.checked_add(wsol_received)
        .ok_or(ErrorCode::MathOverflow)?;
    swap_state.swap_count = swap_state.swap_count.checked_add(1)
        .ok_or(ErrorCode::MathOverflow)?;
    
    if amount_in > 0 {
        // Price is inverted for USDC→SOL (SOL per USDC)
        swap_state.last_swap_price = wsol_received.checked_mul(1_000_000)
            .and_then(|v| v.checked_div(amount_in))
            .ok_or(ErrorCode::MathOverflow)?;
    }
    
    msg!("Swap completed: {} USDC -> {} wSOL", amount_in, wsol_received);
    msg!("Price: {} SOL per USDC", swap_state.last_swap_price);
